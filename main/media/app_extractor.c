...
typedef struct app_extractor_t {
    ...
    // Audio device state flags (replace static variables)
    bool                   audio_dev_opened;
    bool                   audio_decoder_configured;
} app_extractor_t;
...
static esp_err_t process_audio_frame(app_extractor_t *extractor, uint8_t *buffer, uint32_t buffer_size, uint32_t pts)
{
    ...
    if (extractor->audio_format == EXTRACTOR_AUDIO_FORMAT_PCM) {
        if (!extractor->audio_dev_opened) {
            esp_codec_dev_sample_info_t fs = {
                .sample_rate = extractor->audio_sample_rate,
                .channel = extractor->audio_channels,
                .bits_per_sample = extractor->audio_bits
            };
            esp_err_t ret = esp_codec_dev_open(extractor->audio_dev, &fs);
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to open audio device for PCM: %d", ret);
                return ret;
            }
            extractor->audio_dev_opened = true;
            ESP_LOGI(TAG, "Audio device opened for PCM: %dHz, %dch, %dbit", 
                     extractor->audio_sample_rate, extractor->audio_channels, extractor->audio_bits);
        }
    ...
    while (raw.len > 0) {
        ...
        if (out_frame.decoded_size > 0) {
            ...
            if (!extractor->audio_dev_opened) {
                ...
                esp_err_t open_ret = esp_codec_dev_open(extractor->audio_dev, &fs);
                if (open_ret != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to open audio device for decoded audio: %d", open_ret);
                    return open_ret;
                }
                extractor->audio_dev_opened = true;
                ESP_LOGI(TAG, "Audio device opened for decoded audio: %dHz, %dch, %dbit", 
                         extractor->audio_sample_rate, extractor->audio_channels, extractor->audio_bits);
            }
...
static esp_err_t validate_mpeg_compatibility(app_extractor_t *extractor)
{
    if (extractor->has_video) {
        ...
        case EXTRACTOR_VIDEO_FORMAT_H264:
            ESP_LOGE(TAG, "H.264 format not supported - use MJPEG instead");
            return ESP_ERR_NOT_SUPPORTED;

        default:
            ESP_LOGE(TAG, "Unsupported video format %d", extractor->video_format);
            return ESP_ERR_NOT_SUPPORTED;
        }
    }
                             ...
                             app_extractor_handle_t *ret_extractor)
{
    ...
    // Frame rate control initialization
    extractor->frame_interval_ms = 33;  // Default ~30 FPS
    extractor->last_frame_time = 0;

    // Initialize audio state flags
    extractor->audio_dev_opened = false;
    extractor->audio_decoder_configured = false;
    ...
    ESP_LOGI(TAG, "App extractor initialized with %s audio support",
             audio_dev ? "hardware" : "no");
                              ...
                              bool extract_audio)
{
    ...
    // If an audio decoder is already open from a previous file, close it so
    // that the correct decoder can be (re)initialized for the new stream.
    if (extractor->audio_decoder_open) {
        esp_audio_dec_close(extractor->audio_decoder);
        extractor->audio_decoder = NULL;
        extractor->audio_decoder_open = false;
    ...
    // Reset audio state flags for new file
    extractor->audio_dev_opened = false;
    extractor->audio_decoder_configured = false;
...
esp_err_t app_extractor_stop(app_extractor_handle_t handle)
{
    ...
    // Close audio device to reset I2S configuration
    if (extractor->audio_dev && extractor->audio_dev_opened) {
        esp_err_t ret = esp_codec_dev_close(extractor->audio_dev);
        if (ret != ESP_OK) {
            ESP_LOGW(TAG, "Failed to close audio device: %d", ret);
        } else {
            ESP_LOGI(TAG, "Audio device closed successfully");
        }
        extractor->audio_dev_opened = false;
    }
    ...
    // Close the audio decoder to release resources and avoid format mismatch
    if (extractor->audio_decoder_open) {
        esp_audio_dec_close(extractor->audio_decoder);
        extractor->audio_decoder = NULL;
        extractor->audio_decoder_open = false;
    ...
    // Reset audio state flags for next file
    extractor->audio_decoder_configured = false;
